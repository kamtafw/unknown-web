...
import { profileFeedKeys } from "@/lib/socials/query-keys"
...
export function useUserReplies(id: string, enabled = true) {
	...
	return useInfiniteQuery({
		...
		queryFn: async ({ pageParam }) => {
			const res = await socialsApi.getUserRepliesByPath(pageParam as string)
			return {
				replies: res.results as SocialContent[],
				nextPage: res.next as string | null,
			}
		},
		initialPageParam: base as string,
