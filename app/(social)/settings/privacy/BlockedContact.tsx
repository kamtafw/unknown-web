...
interface BlockedContactsPageProps {
  onBack: () => void;
  onNavigate: (view: string, selectedCount?: number) => void;
  setBlockedCount: (count: number) => void;
  blockedCount: number;
}

export default function BlockedContactsPage({ onBack, onNavigate}: BlockedContactsPageProps) {
  ...
  const handleSave = () => {
    if (selectedContacts.length > 0) {
      onNavigate("unblockContactPopup", selectedContacts.length);
    }
  };
