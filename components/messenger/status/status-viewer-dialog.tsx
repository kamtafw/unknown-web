...
import type { StatusListEntry } from "@/lib/messenger/status-grouping"
...
/**
 * Simplification worth stating: progress bars are flat fills, not
 * mobile's decorative multi-segment rings elsewhere in the list. Tap-left
 * = previous, tap-right = next, hold-anywhere = pause — matches mobile's
 * interaction model. Exhausting a user's last story closes the viewer
 * rather than auto-advancing to the next user (no confirmed evidence
 * either way for that behavior, so not guessing at it).
 */
export function StatusViewerDialog({ entry, isOwn, onClose }: StatusViewerDialogProps) {
	...
	const isMutedNow = useStatusMuteStore((s) => s.isMuted(entry.user.pkid))
	...
	return (
		<div className="fixed inset-0 z-200 bg-black flex flex-col">
					...
					<DropdownMenu.Root>
						...
						<DropdownMenu.Portal>
							<DropdownMenu.Content
								align="end"
								sideOffset={4}
								className="z-250 min-w-44 bg-popover border border-border rounded-2xl p-1.5 shadow-xl"
							>
								<DropdownMenu.Item
			...
			{confirmDelete && (
				<div className="absolute inset-0 z-260 bg-black/70 flex items-center justify-center px-6">
						...
						<div className="flex gap-2">
							...
							<button
								onClick={() => {
									...
									if (entry.stories.length <= 1) onClose()
									else goNext()
								}}
